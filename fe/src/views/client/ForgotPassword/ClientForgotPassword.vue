<template>
    <!-- component -->
    <div class=" bg-gray-100 flex flex-col justify-center sm:py-16">
        <div class="p-10 xs:p-0 mx-auto md:w-full md:max-w-md">
            <h1 class="font-bold text-gray-600 text-center text-2xl mb-5">DApp Forgot Password </h1>
            <div class="bg-white shadow-xl w-full rounded-lg divide-y divide-gray-200">
                <div class="px-5 py-7">
                    ...
                    <div v-if="checkNextAction == 'login'">
                        <button type="button"
                            class="transition duration-200 bg-pink-500 hover:bg-pink-600 focus:bg-pink-700 focus:shadow-sm focus:ring-4 focus:ring-pink-500 focus:ring-opacity-50 text-white w-full py-2.5 rounded-lg text-sm shadow-sm hover:shadow-md font-semibold text-center inline-block"
                            v-on:click="handleForgotPassword">
                            <div v-if="!isLoading"><span class="inline-block mr-2">Confirm and Update Password</span>
...
const handleGetVerifyCode = async () => {
    if (email.value) {
        ...
        store.dispatch('getVerifyCodeForgotPassword', dataInput)
        checkNextAction.value = 'login'
    }
}

const handleForgotPassword = () => {
    if (email.value && password.value && verifyCodeSignUp.value) {
        const dataInput = {
        ...
        store.dispatch('forgotPassword', dataInput)
    }
