<template>
    ...
    <div class="min-h-screen bg-gray-100 flex flex-col justify-center sm:py-12">
        <div class="p-10 xs:p-0 mx-auto md:w-full md:max-w-md">
            <h1 class="font-bold text-gray-600 text-center text-2xl mb-5">DApp Signup </h1>
            <div class="bg-white shadow-xl w-full rounded-lg divide-y divide-gray-200">
                <div class="px-5 py-7">
                    <label class="font-semibold text-sm text-gray-600 pb-1 block">Your name</label>
                    <input type="text" class="border rounded-lg px-3 py-2 mt-1 mb-5 text-sm w-full" v-model="username" />
                    ...
                    <label class="font-semibold text-sm text-gray-600 pb-1 block">Verify code in your mailbox</label>
                    <input type="text" class="border rounded-lg px-3 py-2 mt-1 mb-5 text-sm w-full"
                        v-model="verifyCodeSignUp" />
                    <div v-if="checkNextAction == 'verifyCode'">
                        <button type="button"
                            ...
                            v-on:click="handleGetVerifyCode">
                            <span class="inline-block mr-2">Get code</span>
                    ...
                    <div v-if="checkNextAction == 'Signup'">
                        <button type="button"
                            class="transition duration-200 bg-pink-500 hover:bg-pink-600 focus:bg-pink-700 focus:shadow-sm focus:ring-4 focus:ring-pink-500 focus:ring-opacity-50 text-white w-full py-2.5 rounded-lg text-sm shadow-sm hover:shadow-md font-semibold text-center inline-block"
                            v-on:click="handleSignUp">

                            <div v-if="!isLoading">
                                <span class="inline-block mr-2">Confirm and Signup</span>
...
const email = ref('')
const password = ref('')
const verifyCodeSignUp = ref('')
const checkNextAction = ref('verifyCode')
const isLoading = ref(false)

const router = useRouter()
const store = useStore()
// console.log(store.state.authClient.user)
...
const handleGetVerifyCode = () => {
    if (email.value) {
        const dataInput = {userEmail:email.value}
        store.dispatch('getVerifyCodeSignUp', dataInput)
        checkNextAction.value = 'Signup'
    }
    // checkNextAction.value = 'Signup'
}

const handleSignUp = () => {
    if (username.value && email.value && password.value && verifyCodeSignUp.value) {
        const dataInput = {
            username: username.value,
            email: email.value,
            password: password.value,
            verifyCode: verifyCodeSignUp.value,
        };

        // console.log(dataInput)
        store.dispatch('signUp', dataInput)
        // .then(()=>{router.push('/login')})
        // .catch((error) => { console.error(`Signup Failed !`, error); router.push('/signup') })
    }
